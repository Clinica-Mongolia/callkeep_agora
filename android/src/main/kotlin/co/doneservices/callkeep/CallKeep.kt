...
private fun isConnectionServiceAvailable(): Boolean {
    ...
    Log.d(TAG, "Build SDK Version is: $version")
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (intent.action) {
                Constants.ACTION_END_CALL -> {
                    channel.invokeMethod("CallKeepPerformEndCallAction", hashMapOf(
                            "callUUID" to attributeMap?.get(Constants.EXTRA_CALL_UUID)
                    ))
                }
                Constants.ACTION_ANSWER_CALL -> {
                    channel.invokeMethod("CallKeepPerformAnswerCallAction", hashMapOf(
                            "callUUID" to attributeMap?.get(Constants.EXTRA_CALL_UUID)
                    ))
                }
